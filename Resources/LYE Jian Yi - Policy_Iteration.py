...
def main():
    ...
    for state in states.values():
        state.policy = random.choice(policy_list)
    print_states_policy(states)
    print("Initial random state policies" + "\n")
    i = 0

    # Loop to continue the Policy Evaluation and Policy Improvement until the Best Policy is found
    while True:
        # Policy Evaluation
        while True:
            for state in states.values():
                ...
                expected_util = 0
                action = actions.get(state.policy)
        ...
        for state in states.values():
            ...
            if state.is_absorb_state: continue
            expected_utils = []
            for new_action in actions.values():
                expected_util = 0
                for t in new_action.transitions:
                    pos_new = tuple(a + b for a, b in zip(state.pos, t.pos_delta))
                    pos_new = pos_new if pos_new in states else state.pos   # new pos if within grid and not wall
                    state_new = states[pos_new]
                    expected_util += t.transition_prob * (state_new.reward + (state_new.value_old * discount_factor))
                expected_utils.append((expected_util, new_action.direction,))
